...
- [ ] 적격 답변자가 3명 미만인 경우에도 차선 답변자라도 찾아내어 3명에게 전달한다.
...
`감사 메모:` 아래 체크 상태는 현재 코드/마이그레이션 점검 기준이다. `Inbox`는 임시 handoff route만 구현되어 있으며 실제 목록 기능은 아니다. `validation/gate` 테스트 파일은 추가되었지만 이 턴에서 실행 통과까지 확인되지는 않았다.

- [x] Supabase anonymous auth 흐름을 연결한다.
- [x] 첫 세션 생성 시 DB trigger로 해당 사용자의 `profiles` row를 생성한다.
- [x] 온보딩 화면에서 `성별 + 관심분야(복수 선택)`를 수집한다.
...
- [x] 고민 작성 화면을 구현한다.
  ...
  - 기본 길이/필수값 검증
  - 전송 버튼 상태 제어
- [x] 고민 제출 서버 API를 구현한다.
  - [x] raw 입력 수신
  - [x] moderation 실행
  - [x] 차단 시 audit만 남기고 제품 row 생성 금지
  - [x] 승인 시 `concerns` row 생성
- [x] 부적절한 고민 차단 UX를 문서 기준으로 맞춘다.
...
- [x] 실제 고민이 라우팅된 경우에만 해당 답변자들에게 푸시 알림과 앱 알림을 생성한다.
...
- [x] 부적절한 답변 차단 UX를 문서 기준으로 맞춘다.
  ...
  - 기존 작성 내용 유지
  - 수정 후 재전송 가능
- [x] 실사용자 고민에 대한 답변 완료 시 게시자에게 알림을 생성하고 푸시를 보낸다.
- [x] 예제 고민에 대한 답변 완료 시 푸시/알림을 생성하지 않는다.

## 10. My concerns와 답변 확인 흐름 구현

- [x] `Post concern` 내부에 `My concerns` 목록을 구현한다.
- [x] 내가 작성한 실제 고민의 상세 화면을 구현한다.
- [x] 내 고민에 달린 답변 목록/상세 화면을 구현한다.
- [x] 알림에서 해당 고민/답변 상세로 진입할 수 있게 한다.
...
- [x] Expo Notifications 권한 요청 및 토큰 등록 흐름을 구현한다.
...
- [x] 가능하면 읽기/쓰기 경로를 Edge Function 중심으로 고정해 정책 우회를 줄인다.
